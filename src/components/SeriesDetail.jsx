...
const SeriesDetail = () => {
  ...
  const getMovie = async () => {
    const response = await api.get(`/tv/${id}?api_key=${api_key}`);
            ...
            <div className=" flex flex-col gap-2">
                  {
                    ...
                    ? movie.seasons.map((item,index)=>{
                      return(<div key={index} className=" py-2 px-3 rounded-md bg-gray-700 text-white">
                        {item.name} - Number Of Episode({item.episode_count})
                      </div>)
                    })
                    : null                 
